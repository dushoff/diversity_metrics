...
map(rev(1:outerreps), function(x){
        start<-Sys.time()
out<-checkplot_inf(flatten(flatten(SADs_list))[[16]], l=0, inds=size, reps=reps)
write.csv(out, paste("/scratch/mr984/SAD16","l",0,"inds", size, "outernew",  x, ".csv", sep="_"), row.names=F)
rm(out)
