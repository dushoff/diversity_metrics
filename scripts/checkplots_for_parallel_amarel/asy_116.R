...
map(rev(1:outerreps), function(x){
        start<-Sys.time()
out<-checkplot_inf(flatten(flatten(SADs_list))[[11]], l=1, inds=size, reps=reps)
write.csv(out, paste("/scratch/mr984/SAD11","l",1,"inds", size, "outernew",  x, ".csv", sep="_"), row.names=F)
rm(out)
