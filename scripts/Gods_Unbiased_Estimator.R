...
s<-map_dfr(seq(-1,1,1), function(l){
    ...
    #number of individuals in sample
            ...
            map_dfr(1:reps, function(x){
                ...
                return(data.frame(true, gods, naive, chao, l, N,S, commsize=sum(a)))})
            })
    })
})
...
errs<-s %>% group_by(l,S,N) %>% mutate(ge=gods-true, ne=naive-true, ce=chao-true) %>% summarize_at(.vars=c("ge", "ne", "ce"), .funs=rootms) %>% gather(est, err,ge, ne, ce)

s %>% filter(S==30) %>% ggplot(aes(true, chao))+geom_point()

(errs %>% ggplot(aes(N, err, color=est))
