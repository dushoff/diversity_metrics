...
rarity_plot <- function(abundance, p){
	...
	rp <- (ggplot(rf2, aes(x=rarity))
		...
		#This makes a line instead of a stack of points
		# + geom_segment(aes(x=rarity, xend=rarity, y=abundance, yend=0)
		#	, size=1.6)
	
		# Transformation stuff (one or both?)
		...
		# make axis lines... it is just a line segment from min to max of the provided data. teh breaks are provided in scale_x_continuous or scale_y_continuous, wich in turn can get them from 
